...
//        stage3=new Stage(new ScreenViewport());
        ...
        if (isdemo){
            ...
            ghosts.add(new Ghosts( ((GdxSplashScreenGame)game).getmanager().get("HLine.png",Texture.class), rightghosts.get(0),rightghostskill.get(0),Gdx.graphics.getWidth()/2 +100,Gdx.graphics.getHeight()/4, 60f , true));
            //ghosts.add(0,g);
    ...
    public int atlastenemy(int idx,float speed){
        ...
        if (ghosts.size()> 1) {
            ...
            if (isleft == 0) {
                ...
                ghosts.add( new Ghosts(isleft,gestureText.get(idx), leftghosts.get(i),leftghostskill.get(i), -200, pos, speed));
                //ghosts.add(isleft, new Ghosts(isleft,list, leftghosts.get(i),leftghostskill.get(i), -200, pos, speed));

            } else {
                ghosts.add( new Ghosts(isleft, gestureText.get(idx), rightghosts.get(i),rightghostskill.get(i), Gdx.graphics.getWidth() + 20, pos, speed));
            }
        } else {
            isleft = ghosts.size();
            if (isleft == 0)
    ...
    public int atlastenemy(List<GestureTexture> gestureList,float speed){
        ...
        if (ghosts.size()> 1) {
            ...
            if (isleft == 0) {
                ...
                //  list.add(gestureText.get(0));
                //list.add(gestureText.get(1));
                ghosts.add( new Ghosts(isleft,gestureList, leftghosts.get(i),leftghostskill.get(i), -200, pos, speed));
                //ghosts.add(isleft, new Ghosts(isleft,list, leftghosts.get(i),leftghostskill.get(i), -200, pos, speed));

            } else {
                ghosts.add( new Ghosts(isleft, gestureList, rightghosts.get(i),rightghostskill.get(i), Gdx.graphics.getWidth() + 20, pos, speed));
            }
        } else {
            isleft = ghosts.size();
            if (isleft == 0)
    ...
    public void update()
    {
        ...
        if(time > spawntime)
        {
            ...
            if (curenttime < 10){
                ...
                SpwanEnemy(speed);
                SpwanEnemy(speed);
            } else if (curenttime > 10 && curenttime < 20 ){
                speed = 40f;
                spawntime = 3;
                getsize = 2;
                SpwanEnemy(speed);
...
public void levelChange(int levelnum)
{
    ...
    {
        backGround = backgroundList.get(levelnum%4);
        mainGhostAtlas=mainghostAtlasList.get(levelnum%4);
        mainGhostAtlasKill=mainghostAtlasListKill.get(levelnum%4);
        curenttime=0;
        spawntime=2;
        mainghostspawntime=50;
        getsize=4;
    ...
    private void act(){
       ...
       // if(ghosts.size()<=0) {return;}
        //int i=0;
        Iterator<Ghosts> it = ghosts.iterator();
        while(it.hasNext()) {
            Ghosts  value = it.next();
            value.setBounds(value.getX(),value.getY(),value.getWidth(),value.getHeight());
            if(wiz.getBounds().overlaps(value.bounds) ){
                countController++;
                if(countController%120==0) {
                    //countController = 0;
                    if (lifeline > 0) {
                        String g =value.getGest();
                        //  System.out.println("lifeline =  " + lifeline + "g=" + g);
                        value.remove();
                        GhostMap.remove(g, value);
                        lifeline = lifeline - 1;
        ...
        if (mainGhost != null && wiz.getBounds().overlaps(mainGhost.bounds)) {
